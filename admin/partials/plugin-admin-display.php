 ...
 * @subpackage Adv_Geoip_Redirect/admin/partials
 */
?>
<div class="wrap">
	<h2>
		<?php _e( 'GeoIP Redirect Settings', 'adv-geoip-redirect' ); ?>
		<button type="submit" class="button button-secondary" id="geoipr_add_new_btn">
			<?php _e( 'Add New Redirect Rule', 'adv-geoip-redirect' ); ?>
		</button>
	</h2>
	...
	<form action="" method="post" id="geoipr_settings_form">
		...
		<?php
			foreach ( Adv_Geoip_Redirect_Util::get_plugin_settings_chk_fields() as $index => $label ) :
				if ( $label !== 'false' ) :
					$chk_id = Adv_Geoip_Redirect_Util::$option_fields[$index]; ?>
							...
							<div class="col-sm-9">
								<div class="form-check">
									<div class="geoipr_chk-slider">
										<input type="checkbox" class="geoipr_chk-slider-checkbox" id="<?php echo esc_attr( $chk_id ); ?>" <?php checked( Adv_Geoip_Redirect_Util::get_option( $chk_id ), 'true' ); ?>>
				...
				<div class="col-sm-9">
					<div class="form-check">
						<select class="form-control" id="redirection_type">
							<option value="301" <?php selected( Adv_Geoip_Redirect_Util::get_option( 'redirection_type' ), '301' ); ?>><?php _e( '301 Moved Permanently', 'adv-geoip-redirect' ); ?></option>
							<option value="302" <?php selected( Adv_Geoip_Redirect_Util::get_option( 'redirection_type' ), '302' ); ?>><?php _e( '302 Moved Temporarily', 'adv-geoip-redirect' ); ?></option>
						</select>
					</div>
				</div>
			</div>
		...
		<h3 class="redirect_rules_heading" <?php echo ! empty( Adv_Geoip_Redirect_Util::get_option( 'redirect_rules' ) ) ? 'style="display:block;"' : 'style="display:none;"'; ?>>
	...
	<div class="metabox-holder">
		<div class="postbox">
			<h3><span><?php _e( 'Debug Log Viewer', 'adv-geoip-redirect' ); ?></span></h3>
			<div class="inside">
				<textarea class="large-text code" readonly rows="5"><?php echo esc_textarea( Adv_Geoip_Redirect_Util::read_debug_log() ); ?></textarea>
			</div><!-- .inside -->
		</div><!-- .postbox -->
		<div class="postbox">
			<h3><span><?php _e( 'Export Settings', 'adv-geoip-redirect' ); ?></span></h3>
			<div class="inside">
				<p><?php _e( 'Export the currently saved plugin settings. This allows you to easily import the configuration into another site.', 'administrator' ); ?></p>
		...
		<div class="postbox">
			<h3><span><?php _e( 'Import Settings', 'adv-geoip-redirect' ); ?></span></h3>
			<div class="inside">
				<p><?php _e( 'Import the plugin settings. Use above form button (on another site maybe) to generate the import file to use in here', 'adv-geoip-redirect' ); ?></p>
	...
	<script type="text/template" id="tmpl-redirect-rules-set">
		<div class="input-group mb-3 geoipr_rules_group_container">
			<div class="input-group-prepend dropdown">
				<button class="btn btn-outline-secondary dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-expanded="false"><?php _e( 'Settings', 'adv-geoip-redirect' ); ?></button>
				<div class="dropdown-menu">
					<label for="pass_url_parameter_{{data.pass_url_parameter_id}}">
						...
						<?php _e( 'Pass URL Parameters Forward', 'adv-geoip-redirect' ); ?>
					 </label>
					...
					<label for="ignore_url_parameter_{{data.ignore_url_parameter_id}}">
						...
						<?php _e( 'Ignore URL Parameters When Check Against', 'adv-geoip-redirect' ); ?>
					 </label>
				</div>
			  </div>
			...
			<select class="form-control geoipr_user_from_chk_condition">
				<option value="from" <# if ( data.FromChkCondition == 'from' ) { #> selected=selected <# } #>><?php _e( 'From', 'adv-geoip-redirect' ); ?></option>
				<option value="not_from" <# if ( data.FromChkCondition == 'not_from' ) { #> selected=selected <# } #>><?php _e( 'Not From', 'adv-geoip-redirect' ); ?></option>
			</select>
			<select class="form-control geoipr_countries_list" multiple="multiple">
				<?php foreach ( Adv_Geoip_Redirect_Util::get_countries() as $code => $country ) : ?>
...
<style type="text/css">
	.geoipr_chk-slider-inner:before {
		content: '<?php _e( "ENABLED", "adv-geoip-redirect" ); ?>';
	}
	.geoipr_chk-slider-inner:after {
		content: '<?php _e( "DISABLED", "adv-geoip-redirect" ); ?>';
	}
</style>

...
class Adv_Geoip_Redirect_Admin
{
	...
	/**
	 * An array representing a nonce verification error notice.
	 *
	 * This array defines the structure and content of a warning notice to be displayed
	 * in the WordPress admin area when a nonce verification fails.
	 *
	 * @since   2.0.0
	 * @access  private
	 * @var 	array		$nonce_error 	An associative array containing the notice class and message.
	 */
	private $nonce_error = array();
	...
	public function enqueue_styles()
	{
		...
		if ( $current_scrn->id !== 'settings_page_adv-geoip-redirect' ) return;
	...
	public function enqueue_scripts()
	{
		...
		wp_localize_script( $this->plugin_name, 'ADV_GEOIP_REDIRECT', array(
			...
			'redirectRules' 	=> Adv_Geoip_Redirect_Util::get_option( 'redirect_rules' ),
		) );
	}
	...
	{
		require ADV_GEOIP_REDIRECT_PLUGIN_PATH . 'admin/partials/plugin-admin-display.php';
	}
	...
	public function admin_init()
	{
		if ( isset( $_POST['geoipr_export_action'] ) && current_user_can( 'manage_options' ) )
		{
			...
			{
				$this->notices[] 		= $this->nonce_error;
			}
			else
			{
				Adv_Geoip_Redirect_Util::export_settings();
			}
		}
		...
		if ( isset( $_POST['geoipr_import_action'] ) && current_user_can( 'manage_options' ) )
		{
			...
			else
			{
				...
				if ( Adv_Geoip_Redirect_Util::import_settings( $import_file ) )
				{
					$this->notices[] 	= array(
		...
		if ( isset( $_POST['geoipr_reset_btn'] ) && $_POST['geoipr_reset_btn'] == '1' )
		{
			...
			else
			{
				...
				{
					Adv_Geoip_Redirect_Util::reset_plugin_settings();
	...
	public function admin_footer_text( $text )
	{
		...
		if ( $current_scrn->id !== 'settings_page_adv-geoip-redirect' )
		{
	...
	public function capture_form_submit()
	{
		...
		else
		{
			...
			if ( Adv_Geoip_Redirect_Util::update_plugin_settings( $_POST ) )
			{
			...
			{
				$response 					= array( 'status' => 'error', 'message' => __( 'Invalid Request! Please Try Again!', 'adv-geoip-redirect' ) );
			}
		}
	...
	{
		$links[] = sprintf( '<a href="%s">%s</a>', admin_url( 'options-general.php?page=adv-geoip-redirect' ), __( 'Settings', 'adv-geoip-redirect' ) );
